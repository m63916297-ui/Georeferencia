...
class SegmentacionReporte:
    ...
    class Categoria:
        SEGURIDAD = {
            ...
            "subcategorias": [
                "Robo/Asalto", "Vandalismo", "Agresión", "Secuestro",
                "Extorsión", "Delincuencia", "Seguridad Vial", "Otro Seguridad"
            ]
        }
        ...
        INFRAESTRUCTURA = {
            ...
            "subcategorias": [
                "Vía dañada", "Bache", "Semáforo dañado", "Alumbrado público",
                "Puente dañado", "Edificio en riesgo", "Obra abandonada", "Otro Infraestructura"
            ]
        }
        ...
        SERVICIOS = {
            ...
            "subcategorias": [
                "Sin agua", "Sin luz", "Sin gas", "Telecomunicaciones",
                "Transporte público", "Recolección basura", "Salud", "Otro Servicios"
            ]
        }
        ...
        AMBIENTAL = {
            ...
            "subcategorias": [
                "Contaminación", "Deforestación", "Residuos peligrosos",
                "Maltrato animal", "Extinción", "Incendio", "Inundación", "Otro Ambiental"
            ]
        }
        ...
        TRANSITO = {
            ...
            "subcategorias": [
                "Accidente", "Congestión", "Señalización", "Estacionamiento indebido",
                "Vehículo abandonado", "Choque", "Atropello", "Otro Tránsito"
            ]
        }
        ...
        EMERGENCIAS = {
            ...
            "subcategorias": [
                "Incendio", "Explosión", "Fuga de gas", "Emergencia médica",
                "Desastre natural", "Evacuación", "Alerta sanitaria", "Otro Emergencia"
            ]
        }
        
        @classmethod
        def todas(cls):
            return [cls.SEGURIDAD, cls.INFRAESTRUCTURA, cls.SERVICIOS, 
                   cls.AMBIENTAL, cls.TRANSITO, cls.EMERGENCIAS]
    ...
    class Severidad:
        ...
        BAJO = {
            ...
            "icono": "🟢",
            "color": "green",
            "descripcion": "Impacto mínimo o косметический",
            "nivel": 1
        }
    ...
    class Estado:
        ...
        RECHAZADO = {"id": "rechazado", "nombre": "Rechazado", "icono": "❌"]
        
        @classmethod
        def todos(cls):
            return [cls.RECIBIDO, cls.VALIDANDO, cls.ASIGNADO, 
                   cls.EN_PROCESO, cls.RESUELTO, cls.CERRADO, cls.RECHAZADO]
    ...
    class Fuente:
        ...
        EMERGENCIAS = {"id": "emergencias", "nombre": "Línea de Emergencias", "icono": "📞"}
        ...
        @classmethod
        def todas(cls):
            return [cls.CIUDADANO, cls.CAMARAS, cls.SENSORES, 
                   cls.EMERGENCIAS, cls.WEB, cls.MOVIL, cls.SOCIAL]
...
class GeoapifyService:
    ...
    def reverse_geocode(self, lat: float, lon: float) -> Optional[Dict]:
        ...
        return {"latitude": lat, "longitude": lon, "address": f"({lat}, {lon})",
    ...
    def get_ip_location(self) -> Optional[Dict]:
        ...
        if data and data.get("location"):
            ...
            return {
                ...
                "address": "", "city": loc.get("city", "") or "Unknown",
...
def render_header():
    ...
    with col1:
        ...
        """, unsafe_allow_html=True)
    with col2:
        st.markdown('<span class="badge badge-provider">🏢 PROVIDER ACTIVO</span>', unsafe_allow_html=True)
        st.markdown('<span class="badge badge-safe">🛡️ SAFE INTELIGENCIA</span>', unsafe_allow_html=True)
...
def create_map(incidents: List[Dict], center: List = None, zoom: int = DEFAULT_ZOOM):
    ...
    for inc in incidents:
        ...
        folium.CircleMarker(
            ...
            fill=True, fillOpacity=0.3, weight=2
        ).add_to(m)
...
def page_report():
    ...
    st.markdown("### Complete el formulario según la Segmentación Estratégica de Variables")
    ...
    with st.form("report_form", clear_on_submit=False):
        ...
        with col1:
            title = st.text_input("Título del Incidente *", placeholder="Descripción breve del incidente")
            category = st.selectbox("Categoría *", [c['id'] for c in SegmentacionReporte.Categoria.todas()],
                                   format_func=lambda x: f"{[c['icono'] for c in SegmentacionReporte.Categoria.todas() if c['id']==x][0]} {[c['nombre'] for c in SegmentacionReporte.Categoria.todas() if c['id']==x][0]}")
            subcategoria = st.selectbox("Subcategoría", [s for c in SegmentacionReporte.Categoria.todas() if c['id']==(category or 'seguridad') for s in c['subcategorias']])
            severity = st.selectbox("Severidad *", [s['id'] for s in SegmentacionReporte.Severidad.todas()],
                                   format_func=lambda x: f"{[s['icono'] for s in SegmentacionReporte.Severidad.todas() if s['id']==x][0]} {[s['nombre'] for s in SegmentacionReporte.Severidad.todas() if s['id']==x][0]}")
        
        with col2:
            reporter = st.text_input("Reportado por *", placeholder="Nombre del reportante")
            contact = st.text_input("Contacto *", placeholder="Teléfono o email")
            fuente = st.selectbox("Fuente del Reporte", [f['id'] for f in SegmentacionReporte.Fuente.todas()],
                                format_func=lambda x: f"{[f['icono'] for f in SegmentacionReporte.Fuente.todas() if f['id']==x][0]} {[f['nombre'] for f in SegmentacionReporte.Fuente.todas() if f['id']==x][0]}")
            priority = st.selectbox("Prioridad", [p['id'] for p in SegmentacionReporte.Prioridad.todas()],
                                  format_func=lambda x: f"{[p['nombre'] for p in SegmentacionReporte.Prioridad.todas() if p['id']==x][0]}")
        
        impact = st.selectbox("Impacto Estimado", [i['id'] for i in SegmentacionReporte.Impacto.todo()],
                            format_func=lambda x: f"{[i['nombre'] for i in SegmentacionReporte.Impacto.todo() if i['id']==x][0]} ({[i['afectados'] for i in SegmentacionReporte.Impacto.todo() if i['id']==x][0]} afectados)")
        
        description = st.text_area("Descripción Detallada *", height=120,
                                  placeholder="Describa el incidente con detalle...")
        
        st.markdown("---")
        st.subheader("📍 Ubicación del Incidente")
        
        method = st.radio("Método de ubicación:", 
        ...
        if method == "🔍 Buscar dirección":
            addr = st.text_input("Dirección:", placeholder="Ej: Av. Jiménez # 4-35, Bogotá")
            if addr and st.form_submit_button("🔍 Buscar en Geoapify"):
                with st.spinner("Consultando API de Geoapify..."):
                    ...
                    if loc:
                        ...
                        st.session_state.map_center = [loc.get('latitude'), loc.get('longitude')]
                    else:
            ...
            if st.form_submit_button("📍 Obtener mi ubicación"):
                with st.spinner("Obteniendo ubicación..."):
                    ...
                    if loc:
                        ...
                        st.session_state.pending_location = loc
                        st.session_state.location_confirmed = True
                        st.success(f"✅ {loc.get('city', '')}, {loc.get('country', '')}")
                        st.session_state.map_center = [loc.get('latitude'), loc.get('longitude')]
        
        elif method == "🗺️ Seleccionar en mapa":
            m = create_map([], st.session_state.map_center)
            clicked = st_folium(m, width=600, height=350, key="sel_map", returned_objects=["last_clicked"])
...
def page_map():
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        f_cat = st.selectbox("Categoría", ["Todas"] + [c['id'] for c in SegmentacionReporte.Categoria.todas()])
    with col2:
        f_sev = st.selectbox("Severidad", ["Todas"] + [s['id'] for s in SegmentacionReporte.Severidad.todas()])
    with col3:
        f_stat = st.selectbox("Estado", ["Todos"] + [e['id'] for e in SegmentacionReporte.Estado.todos()])
    with col4:
        f_pri = st.selectbox("Prioridad", ["Todas"] + [p['id'] for p in SegmentacionReporte.Prioridad.todas()])
...
def page_list():
    ...
    col1, col2 = st.columns(2)
    with col1:
        f_cat = st.selectbox("Filtrar categoría", ["Todas"] + [c['id'] for c in SegmentacionReporte.Categoria.todas()])
    with col2:
        f_stat = st.selectbox("Filtrar estado", ["Todos"] + [e['id'] for e in SegmentacionReporte.Estado.todos()])
    ...
    for i, inc in enumerate(incidents):
        ...
        with st.expander(f"{CATEGORY_EMOJI.get(cat, '📌')} {inc.get('title', '')} | :{SEVERITY_COLORS.get(sev, 'blue')}[{sev.upper()}]"):
            col1, col2, col3 = st.columns(3)
            with col1:
                st.markdown(f"**📁 Categoría:** {inc.get('subcategoria', inc.get('category', ''))}")
            ...
            st.markdown(f"**🗺️ Coordenadas:** ({loc.get('latitude', 0):.6f}, {loc.get('longitude', 0):.6f})")
            
            col_upd, col_del = st.columns(2)
            
            with col_upd:
                new_status = st.selectbox("Cambiar Estado", [e['id'] for e in SegmentacionReporte.Estado.todos()],
                                        index=[e['id'] for e in SegmentacionReporte.Estado.todos()].index(inc.get('status', 'recibido')) 
                                        if inc.get('status', 'recibido') in [e['id'] for e in SegmentacionReporte.Estado.todos()] else 0,
